...
const inicio = () => {
  ...
  if (partidas.length > 0) {
    ...
    partidas.forEach((p, index) => {
      listaPartidas.innerHTML += `<li onclick='verPartida(${index})'>${
        index + 1
      } - ${p.p1} vs ${p.p2}<i class="fa-solid fa-magnifying-glass"></i></li>`
    })
  }
}
...
const iniciarJuego = () => {
  ...
  if (p1 && p2) {
    ...
    lbl.innerHTML = 'INSERT YOUR NAMES PLEASE'
  }
}
...
const mostrarCartas = (hayPartida) => {
  ...
  cartasJuego.forEach((carta, index) => {
    ...
    carousel.innerHTML += `<div class="carousel-item ${active}" data-id=${carta.id} data-index=${playerIndex} data-name='${playerName}'>
...
const cambiarTitulos = (carta, index, jugador) => {
  cardTitle.innerHTML = `Card ${index} of "${jugador}"`
...
const verResultados = () => {
  ...
  if (partidaCargada) btnGuardar.style.display = 'none'
  else btnGuardar.style.display = 'inline-block'
  ...
  `
  resultado.innerHTML = partida.resultado
    ? `TIENEN MISTICA`
    : `NO TIENEN MISTICA`
}
